...
public class TopupScreenScanner extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        appTimeOutManagerUtil = new AppTimeOutManagerUtil(TopupScreenScanner.this);
        ...
        topUpQRScannerUtils = new TopUpQRScannerUtils(surfaceView, this, merchantId) {
            ...
            public void parsedScanData(HashMap<String, String> stringHashMap, String outletId, String authToken, String campaignId) {
                ...
                if (topup == null || topup.size() <= 0) {
                    new AlertDialogFailure(TopupScreenScanner.this, "Campaigns not found", "OK", "Failed") {
                        @Override
            ...
            public void onFailureScan(final String result) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        new AlertDialogFailure(TopupScreenScanner.this, result, "OK", "Failed") {
                            @Override
                            public void onButtonClick() {
                                onBackPressed();
                            }
    ...
    private void callApi() {
        if (Util.getConnectivityStatusString(TopupScreenScanner.this)) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    if (outletID != null && authTokenVAlue != null) {
                        ...
                        coralsToast = new MerchantToast(TopupScreenScanner.this);
            ...
            new AlertDialogFailure(TopupScreenScanner.this, "Please turn ON your data connection", "Retry", "No internet Connection !") {
                @Override
                public void onButtonClick() {
                    callApi();
                }
    ...
    private void callWebService(final String outletId, final String authToken) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                try {
                    intermediateAlertDialog = new IntermediateAlertDialog(TopupScreenScanner.this);
    ...
    public void onBackPressed() {
        startActivity(new Intent(TopupScreenScanner.this, Homenew.class));
        finish();
    ...
    private void callTopUpApi(String outletId, String authToken) throws ApiException {
        ...
        clientApisApi.customerTopupPostAsync(body7, new ApiCallback<InlineResponse2007>() {
            ...
            public void onFailure(ApiException e, int statusCode, Map<String, List<String>> responseHeaders) {
                ...
                runOnUiThread(() -> new AlertDialogFailure(TopupScreenScanner.this, "Please try again later", "OK", "Top up failed") {
                    @Override
                    public void onButtonClick() {
                        onBackPressed();
                    }
            ...
            public void onSuccess(InlineResponse2007 result, int statusCode, Map<String, List<String>> responseHeaders) {
                if (statusCode == 200) {
                    if (result != null) {
                        ...
                        new AlertDialogFailure(TopupScreenScanner.this, "Please try again later", "OK", "Top up failed") {
                            @Override
                            public void onButtonClick() {
                                onBackPressed();
                            }
    ...
    private void successIntent(String wallBal) {
        startActivity(new Intent(this, TopupScreenSuccess.class)
